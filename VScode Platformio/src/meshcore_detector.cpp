...
static std::vector<MeshCoreDeviceData> meshcoreDevices;
...
bool hasMeshCoreServiceUUID(uint8_t *adv_data, uint8_t adv_data_len) {
    uint8_t uuid_len = 0;
    ...
    if (uuid_data != NULL && uuid_len >= 16) {
        for (int i = 0; i + 16 <= uuid_len; i += 16) {
            if (memcmp(&uuid_data[i], MESHCORE_SERVICE_UUID, 16) == 0) {
                return true;
            }
        }
    }
...
bool hasMeshCoreName(uint8_t *adv_data, uint8_t adv_data_len) {
    uint8_t adv_name_len = 0;
    uint8_t *adv_name = esp_ble_resolve_adv_data(adv_data, ESP_BLE_AD_TYPE_NAME_CMPL, &adv_name_len);
    ...
    if (adv_name != NULL && adv_name_len >= 9) {
        char nameBuf[32];
        uint8_t copyLen = (adv_name_len < 31) ? adv_name_len : 31;
        memcpy(nameBuf, adv_name, copyLen);
        nameBuf[copyLen] = '\0';

        return (strncmp(nameBuf, "MeshCore-", 9) == 0);
    }
...
static void process_scan_result(esp_ble_gap_cb_param_t *scan_result) {
    ...
    for (size_t i = 0; i < meshcoreDevices.size(); i++) {
        if (strcmp(meshcoreDevices[i].address, addrStr) == 0) {

            meshcoreDevices[i].rssi = scan_result->scan_rst.rssi;
            meshcoreDevices[i].lastSeen = millis();
            ...
            if (adv_name != NULL && adv_name_len > 0 && adv_name_len < 32) {
                char tempName[32];
                memcpy(tempName, adv_name, adv_name_len);
                tempName[adv_name_len] = '\0';

                if (strncmp(tempName, "MeshCore-", 9) == 0) {
                    memcpy(meshcoreDevices[i].name, tempName, adv_name_len + 1);
                    meshcoreDevices[i].hasName = true;
                } else if (!meshcoreDevices[i].hasName) {
                    meshcoreDevices.erase(meshcoreDevices.begin() + i);
                    return;
                }
            }

            if (!isLocateMode) {
                std::sort(meshcoreDevices.begin(), meshcoreDevices.end(),
                          [](const MeshCoreDeviceData &a, const MeshCoreDeviceData &b) {
    ...
    bool hasUUID = hasMeshCoreServiceUUID(scan_result->scan_rst.ble_adv,
    ...
    } else if (meshcoreDevices.size() >= MAX_DEVICES) {
        return;
    }

    MeshCoreDeviceData newDev = {};
    strncpy(newDev.address, addrStr, 17);
    newDev.address[17] = '\0';
    newDev.rssi = scan_result->scan_rst.rssi;
    newDev.lastSeen = millis();
    newDev.isMeshCore = true;

    strcpy(newDev.name, "MeshCore");
    ...
    if (adv_name != NULL && adv_name_len > 0 && adv_name_len < 32) {
        ...
        if (strncmp(tempName, "MeshCore-", 9) == 0) {
            memcpy(newDev.name, tempName, adv_name_len + 1);
            newDev.hasName = true;
        }
    }

    meshcoreDevices.push_back(newDev);
...
void meshcoreDetectorSetup() {
    meshcoreDevices.clear();
    meshcoreDevices.reserve(MAX_DEVICES);
...
void meshcoreDetectorLoop() {
    if (!bleInitialized) return;

    unsigned long now = millis();

    if (isScanning && !isLocateMode) {
        if (lastDeviceCount != (int)meshcoreDevices.size() || wasScanning != isScanning) {
            lastDeviceCount = (int)meshcoreDevices.size();
            ...
            char countStr[32];
            snprintf(countStr, sizeof(countStr), "%d/%d devices", (int)meshcoreDevices.size(), MAX_DEVICES);
            u8g2.drawStr(0, 35, countStr);
            ...
            int fillWidth = (meshcoreDevices.size() * (barWidth - 4)) / MAX_DEVICES;
            if (fillWidth > 0) {
    ...
    if (meshcoreDevices.empty() && isContinuousScanEnabled()) {
    ...
    if (!isScanning && scanCompleted && now - lastScanTime > effectiveScanInterval &&
        ...
        if (meshcoreDevices.size() >= MAX_DEVICES) {
            std::sort(meshcoreDevices.begin(), meshcoreDevices.end(),
                    [](const MeshCoreDeviceData &a, const MeshCoreDeviceData &b) {
            ...
            if (devicesToRemove > 0) {
                meshcoreDevices.erase(meshcoreDevices.begin(),
                                        meshcoreDevices.begin() + devicesToRemove);
            }
    ...
    if (scanCompleted && now - lastButtonPress > debounceTime) {
        if (!isDetailView && !isLocateMode && digitalRead(BTN_UP) == LOW && currentIndex > 0) {
            ...
            needsRedraw = true;
                   ...
                   currentIndex < (int)meshcoreDevices.size() - 1) {
            ++currentIndex;
            ...
            needsRedraw = true;
                   ...
                   !meshcoreDevices.empty()) {
            isLocateMode = true;
            strncpy(locateTargetAddress, meshcoreDevices[currentIndex].address, sizeof(locateTargetAddress) - 1);
    ...
    if (meshcoreDevices.empty()) {
        ...
        currentIndex = constrain(currentIndex, 0, (int)meshcoreDevices.size() - 1);
        listStartIndex =
            constrain(listStartIndex, 0, max(0, (int)meshcoreDevices.size() - 5));
    }
    ...
    if (meshcoreDevices.empty() && scanCompleted && !isScanning &&
    ...
    if (meshcoreDevices.empty()) {
        if (isContinuousScanEnabled()) {
            ...
            u8g2.setFont(u8g2_font_6x10_tr);
            u8g2.drawStr(0, 10, "No MeshCore found");
            u8g2.setFont(u8g2_font_5x8_tr);
            char timeStr[32];
        ...
        auto &dev = meshcoreDevices[currentIndex];
        u8g2.setFont(u8g2_font_5x8_tr);
        char buf[32];
        ...
        u8g2.setFont(u8g2_font_6x10_tr);
        char header[32];
        snprintf(header, sizeof(header), "MeshCore: %d/%d",
                 (int)meshcoreDevices.size(), MAX_DEVICES);
        u8g2.drawStr(0, 10, header);

        for (int i = 0; i < 5; ++i) {
            int idx = listStartIndex + i;
            if (idx >= (int)meshcoreDevices.size())
                break;
            auto &d = meshcoreDevices[idx];
            if (idx == currentIndex)
                u8g2.drawStr(0, 20 + i * 10, ">");
            char line[32];
            const char* displayName = d.name[0] ? d.name : "MeshCore";
            char maskedName[33];
