...
static std::vector<SmartTagDeviceData> smarttagDevices;
...
bool hasSmartTagServiceUUID(uint8_t *adv_data, uint8_t adv_data_len) {
    uint8_t uuid16_len = 0;
    uint8_t *uuid16_data = esp_ble_resolve_adv_data(adv_data, ESP_BLE_AD_TYPE_16SRV_CMPL, &uuid16_len);

    if (uuid16_data != NULL && uuid16_len >= 2) {
        for (int i = 0; i + 2 <= uuid16_len; i += 2) {
            uint16_t uuid16 = uuid16_data[i] | (uuid16_data[i + 1] << 8);
            if (uuid16 == 0xFD5A) {
                return true;
            }
        }
    ...
    uuid16_len = 0;
    uuid16_data = esp_ble_resolve_adv_data(adv_data, ESP_BLE_AD_TYPE_16SRV_PART, &uuid16_len);
...
static void process_scan_result(esp_ble_gap_cb_param_t *scan_result) {
    ...
    for (size_t i = 0; i < smarttagDevices.size(); i++) {
        if (strcmp(smarttagDevices[i].address, addrStr) == 0) {
            smarttagDevices[i].rssi = scan_result->scan_rst.rssi;
            smarttagDevices[i].lastSeen = now;
            ...
            if (adv_name != NULL && adv_name_len > 0 && adv_name_len < 32) {
                memcpy(smarttagDevices[i].name, adv_name, adv_name_len);
                smarttagDevices[i].name[adv_name_len] = '\0';
            }

            if (!isLocateMode) {
                std::sort(smarttagDevices.begin(), smarttagDevices.end(),
                          [](const SmartTagDeviceData &a, const SmartTagDeviceData &b) {
    ...
    bool isSmartTagDevice = hasSmartTagServiceUUID(scan_result->scan_rst.ble_adv,
    ...
    } else if (smarttagDevices.size() >= MAX_DEVICES) {
        return;
    }

    if (isLocateMode) return;

    SmartTagDeviceData newDev = {};
    strncpy(newDev.address, addrStr, 17);
    newDev.address[17] = '\0';
    newDev.rssi = scan_result->scan_rst.rssi;
    newDev.lastSeen = now;
    newDev.isSmartTag = true;
    strcpy(newDev.name, "SmartTag");
    ...
    smarttagDevices.push_back(newDev);
...
void smarttagDetectorSetup() {
    smarttagDevices.clear();
    smarttagDevices.reserve(MAX_DEVICES);
    ...
    u8g2.drawStr(0, 20, "SmartTag devices...");
    char countStr[32];
...
void smarttagDetectorLoop() {
    ...
    if (isScanning && !isLocateMode) {
        if (lastDeviceCount != (int)smarttagDevices.size() || wasScanning != isScanning) {
            lastDeviceCount = (int)smarttagDevices.size();
            ...
            char countStr[32];
            snprintf(countStr, sizeof(countStr), "%d/%d devices", (int)smarttagDevices.size(), MAX_DEVICES);
            u8g2.drawStr(0, 35, countStr);
            ...
            int fillWidth = (smarttagDevices.size() * (barWidth - 4)) / MAX_DEVICES;
            if (fillWidth > 0) {
    ...
    if (smarttagDevices.empty() && isContinuousScanEnabled()) {
    ...
    if (!isScanning && scanCompleted && now - lastScanTime > effectiveScanInterval &&
        ...
        if (smarttagDevices.size() >= MAX_DEVICES) {
            std::sort(smarttagDevices.begin(), smarttagDevices.end(),
                    [](const SmartTagDeviceData &a, const SmartTagDeviceData &b) {
            ...
            if (devicesToRemove > 0) {
                smarttagDevices.erase(smarttagDevices.begin(),
                                smarttagDevices.begin() + devicesToRemove);
            }
    ...
    if (scanCompleted && now - lastButtonPress > debounceTime) {
        if (!isDetailView && !isLocateMode && digitalRead(BTN_UP) == LOW && currentIndex > 0) {
            ...
            needsRedraw = true;
                   ...
                   currentIndex < (int)smarttagDevices.size() - 1) {
            ++currentIndex;
            ...
            needsRedraw = true;
                   ...
                   !smarttagDevices.empty()) {
            isLocateMode = true;
            strncpy(locateTargetAddress, smarttagDevices[currentIndex].address, sizeof(locateTargetAddress) - 1);
    ...
    if (smarttagDevices.empty()) {
        ...
        currentIndex = constrain(currentIndex, 0, (int)smarttagDevices.size() - 1);
        listStartIndex =
            constrain(listStartIndex, 0, max(0, (int)smarttagDevices.size() - 5));
    }
    ...
    if (smarttagDevices.empty() && scanCompleted && !isScanning &&
    ...
    if (smarttagDevices.empty()) {
        if (isContinuousScanEnabled()) {
            ...
            u8g2.setFont(u8g2_font_6x10_tr);
            u8g2.drawStr(0, 10, "No SmartTags found");
            u8g2.setFont(u8g2_font_5x8_tr);
            char timeStr[32];
        ...
        auto &dev = smarttagDevices[currentIndex];
        u8g2.setFont(u8g2_font_5x8_tr);
        char buf[32];
        ...
        u8g2.setFont(u8g2_font_6x10_tr);
        char header[32];
        snprintf(header, sizeof(header), "SmartTags: %d/%d", (int)smarttagDevices.size(), MAX_DEVICES);
        u8g2.drawStr(0, 10, header);
        for (int i = 0; i < 5; ++i) {
            int idx = listStartIndex + i;
            if (idx >= (int)smarttagDevices.size())
                break;
            auto &d = smarttagDevices[idx];
            if (idx == currentIndex)
