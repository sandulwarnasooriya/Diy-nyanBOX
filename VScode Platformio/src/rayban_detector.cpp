...
static std::vector<RayBanDeviceData> raybanDevices;
...
bool hasRayBanServiceUUID(uint8_t *adv_data, uint8_t adv_data_len) {
...
static void process_scan_result(esp_ble_gap_cb_param_t *scan_result) {
    ...
    for (size_t i = 0; i < raybanDevices.size(); i++) {
        if (strcmp(raybanDevices[i].address, addrStr) == 0) {
            raybanDevices[i].rssi = scan_result->scan_rst.rssi;
            raybanDevices[i].lastSeen = now;
            ...
            if (adv_name != NULL && adv_name_len > 0 && adv_name_len < 32) {
                memcpy(raybanDevices[i].name, adv_name, adv_name_len);
                raybanDevices[i].name[adv_name_len] = '\0';
            }

            if (!isLocateMode) {
                std::sort(raybanDevices.begin(), raybanDevices.end(),
                          [](const RayBanDeviceData &a, const RayBanDeviceData &b) {
    ...
    bool isRayBanDevice = hasRayBanServiceUUID(scan_result->scan_rst.ble_adv,
    ...
    } else if (raybanDevices.size() >= MAX_DEVICES) {
        return;
    }

    if (isLocateMode) return;

    RayBanDeviceData newDev = {};
    strncpy(newDev.address, addrStr, 17);
    newDev.address[17] = '\0';
    newDev.rssi = scan_result->scan_rst.rssi;
    newDev.lastSeen = now;
    newDev.isRayBan = true;
    strcpy(newDev.name, "RayBan Device");
    ...
    raybanDevices.push_back(newDev);
...
void raybanDetectorSetup() {
    raybanDevices.clear();
    raybanDevices.reserve(MAX_DEVICES);
    ...
    u8g2.drawStr(0, 20, "RayBan devices...");
    char countStr[32];
...
void raybanDetectorLoop() {
    if (!bleInitialized) return;

    unsigned long now = millis();

    if (isScanning && !isLocateMode) {
        if (lastDeviceCount != (int)raybanDevices.size() || wasScanning != isScanning) {
            lastDeviceCount = (int)raybanDevices.size();
            ...
            char countStr[32];
            snprintf(countStr, sizeof(countStr), "%d/%d devices", (int)raybanDevices.size(), MAX_DEVICES);
            u8g2.drawStr(0, 35, countStr);
            ...
            int fillWidth = (raybanDevices.size() * (barWidth - 4)) / MAX_DEVICES;
            if (fillWidth > 0) {
    ...
    if (raybanDevices.empty() && isContinuousScanEnabled()) {
    ...
    if (!isScanning && scanCompleted && now - lastScanTime > effectiveScanInterval &&
        ...
        if (raybanDevices.size() >= MAX_DEVICES) {
            std::sort(raybanDevices.begin(), raybanDevices.end(),
                    [](const RayBanDeviceData &a, const RayBanDeviceData &b) {
            ...
            if (devicesToRemove > 0) {
                raybanDevices.erase(raybanDevices.begin(),
                                raybanDevices.begin() + devicesToRemove);
            }
    ...
    if (scanCompleted && now - lastButtonPress > debounceTime) {
        if (!isDetailView && !isLocateMode && digitalRead(BTN_UP) == LOW && currentIndex > 0) {
            ...
            needsRedraw = true;
                   ...
                   currentIndex < (int)raybanDevices.size() - 1) {
            ++currentIndex;
            ...
            needsRedraw = true;
                   ...
                   !raybanDevices.empty()) {
            isLocateMode = true;
            strncpy(locateTargetAddress, raybanDevices[currentIndex].address, sizeof(locateTargetAddress) - 1);
    ...
    if (raybanDevices.empty()) {
        ...
        currentIndex = constrain(currentIndex, 0, (int)raybanDevices.size() - 1);
        listStartIndex =
            constrain(listStartIndex, 0, max(0, (int)raybanDevices.size() - 5));
    }
    ...
    if (raybanDevices.empty() && scanCompleted && !isScanning &&
    ...
    if (raybanDevices.empty()) {
        if (isContinuousScanEnabled()) {
            ...
            u8g2.setFont(u8g2_font_6x10_tr);
            u8g2.drawStr(0, 10, "No RayBans found");
            u8g2.setFont(u8g2_font_5x8_tr);
            char timeStr[32];
        ...
        auto &dev = raybanDevices[currentIndex];
        u8g2.setFont(u8g2_font_5x8_tr);
        char buf[32];
        ...
        u8g2.setFont(u8g2_font_6x10_tr);
        char header[32];
        snprintf(header, sizeof(header), "RayBans: %d/%d", (int)raybanDevices.size(), MAX_DEVICES);
        u8g2.drawStr(0, 10, header);
        for (int i = 0; i < 5; ++i) {
            int idx = listStartIndex + i;
            if (idx >= (int)raybanDevices.size())
                break;
            auto &d = raybanDevices[idx];
            if (idx == currentIndex)
