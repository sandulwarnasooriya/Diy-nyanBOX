...
static void process_scan_result(esp_ble_gap_cb_param_t *scan_result) {
    ...
    } else if (skimmerDevices.size() >= MAX_DEVICES) {
        return;
    }
...
void cardskimmerDetectorLoop() {
    ...
    if (isScanning && !isLocateMode) {
        if (lastDeviceCount != (int)skimmerDevices.size() || wasScanning != isScanning) {
            lastDeviceCount = (int)skimmerDevices.size();
            ...
            char countStr[32];
            snprintf(countStr, sizeof(countStr), "%d/%d devices", (int)skimmerDevices.size(), MAX_DEVICES);
            u8g2.drawStr(0, 35, countStr);
            ...
            int fillWidth = (skimmerDevices.size() * (barWidth - 4)) / MAX_DEVICES;
            if (fillWidth > 0) {
    ...
    if (scanCompleted && now - lastButtonPress > debounceTime) {
        if (!isDetailView && !isLocateMode && digitalRead(BTN_UP) == LOW && currentIndex > 0) {
            ...
            needsRedraw = true;
                   ...
                   !skimmerDevices.empty()) {
            isLocateMode = true;
            strncpy(locateTargetAddress, skimmerDevices[currentIndex].address, sizeof(locateTargetAddress) - 1);
    ...
    if (skimmerDevices.empty() && scanCompleted && !isScanning &&
    ...
    if (skimmerDevices.empty()) {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            char line[32];
            const char* displayName = d.name[0] ? d.name : "Skimmer";
            char maskedName[33];
