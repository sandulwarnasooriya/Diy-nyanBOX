...
static std::vector<TileDeviceData> tileDevices;
...
bool hasTileServiceUUID(uint8_t *adv_data, uint8_t adv_data_len) {
    ...
    if (uuid16_data != NULL && uuid16_len >= 2) {
        for (int i = 0; i + 2 <= uuid16_len; i += 2) {
            ...
            if (uuid16 == 0xfeed || uuid16 == 0xfeec) {
                return true;
            }
        }
    }
...
static void process_scan_result(esp_ble_gap_cb_param_t *scan_result) {
    ...
    for (size_t i = 0; i < tileDevices.size(); i++) {
        if (strcmp(tileDevices[i].address, addrStr) == 0) {
            tileDevices[i].rssi = scan_result->scan_rst.rssi;
            tileDevices[i].lastSeen = now;
            ...
            if (adv_name != NULL && adv_name_len > 0 && adv_name_len < 32) {
                memcpy(tileDevices[i].name, adv_name, adv_name_len);
                tileDevices[i].name[adv_name_len] = '\0';
                tileDevices[i].hasName = true;
            }

            if (!isLocateMode) {
                std::sort(tileDevices.begin(), tileDevices.end(),
                          [](const TileDeviceData &a, const TileDeviceData &b) {
    ...
    bool isTileDevice = hasTileServiceUUID(scan_result->scan_rst.ble_adv,
    ...
    } else if (tileDevices.size() >= MAX_DEVICES) {
        return;
    }

    if (isLocateMode) return;

    TileDeviceData newDev = {};
...
void tileDetectorSetup() {
    tileDevices.clear();
    tileDevices.reserve(MAX_DEVICES);
    ...
    u8g2.drawStr(0, 20, "Tile devices...");
    char countStr[32];
...
void tileDetectorLoop() {
    ...
    if (isScanning && !isLocateMode) {
        if (lastDeviceCount != (int)tileDevices.size() || wasScanning != isScanning) {
            lastDeviceCount = (int)tileDevices.size();
            ...
            char countStr[32];
            snprintf(countStr, sizeof(countStr), "%d/%d devices", (int)tileDevices.size(), MAX_DEVICES);
            u8g2.drawStr(0, 35, countStr);
            ...
            int fillWidth = (tileDevices.size() * (barWidth - 4)) / MAX_DEVICES;
            if (fillWidth > 0) {
    ...
    if (tileDevices.empty() && isContinuousScanEnabled()) {
    ...
    if (!isScanning && scanCompleted && now - lastScanTime > effectiveScanInterval &&
        ...
        if (tileDevices.size() >= MAX_DEVICES) {
            std::sort(tileDevices.begin(), tileDevices.end(),
                    [](const TileDeviceData &a, const TileDeviceData &b) {
            ...
            if (devicesToRemove > 0) {
                tileDevices.erase(tileDevices.begin(),
                                tileDevices.begin() + devicesToRemove);
            }
    ...
    if (scanCompleted && now - lastButtonPress > debounceTime) {
        if (!isDetailView && !isLocateMode && digitalRead(BTN_UP) == LOW && currentIndex > 0) {
            ...
            needsRedraw = true;
                   ...
                   currentIndex < (int)tileDevices.size() - 1) {
            ++currentIndex;
            ...
            needsRedraw = true;
                   ...
                   !tileDevices.empty()) {
            isLocateMode = true;
            strncpy(locateTargetAddress, tileDevices[currentIndex].address, sizeof(locateTargetAddress) - 1);
    ...
    if (tileDevices.empty()) {
        ...
        currentIndex = constrain(currentIndex, 0, (int)tileDevices.size() - 1);
        listStartIndex =
            constrain(listStartIndex, 0, max(0, (int)tileDevices.size() - 5));
    }
    ...
    if (tileDevices.empty() && scanCompleted && !isScanning &&
    ...
    if (tileDevices.empty()) {
        if (isContinuousScanEnabled()) {
            ...
            u8g2.setFont(u8g2_font_6x10_tr);
            u8g2.drawStr(0, 10, "No Tiles found");
            u8g2.setFont(u8g2_font_5x8_tr);
            char timeStr[32];
        ...
        auto &dev = tileDevices[currentIndex];
        u8g2.setFont(u8g2_font_5x8_tr);
        char buf[32];
        ...
        u8g2.setFont(u8g2_font_6x10_tr);
        char header[32];
        snprintf(header, sizeof(header), "Tiles: %d/%d", (int)tileDevices.size(), MAX_DEVICES);
        u8g2.drawStr(0, 10, header);
        for (int i = 0; i < 5; ++i) {
            int idx = listStartIndex + i;
            if (idx >= (int)tileDevices.size())
                break;
            auto &d = tileDevices[idx];
            if (idx == currentIndex)
                u8g2.drawStr(0, 20 + i * 10, ">");
            char line[32];
            const char* displayName = d.hasName && d.name[0] ? d.name : "Tile";
            char maskedName[33];
